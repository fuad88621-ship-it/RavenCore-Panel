...
// Like agentRequest but sends a raw octet-stream body (used to relay file
// transfers between agents through the panel). `body` can be a Buffer or a web
// ReadableStream. Returns parsed JSON.
export async function agentRequestStream(path, method = 'POST', body, opts = {}) {
  const node = opts.node || null;
  const isLocal = node && (node.name === config.node.name || node.fqdn === config.node.fqdn);
  const base = node && !isLocal
    ? `${node.scheme || 'http'}://${node.fqdn}:${node.port}`
    : agentInternalUrl;
  const token = node && !isLocal ? node.daemon_token : config.security.agent_token;
  const isStream = typeof body?.pipeThrough === 'function' || typeof body?.getReader === 'function';
  const res = await fetch(`${base}${path}`, {
    method,
    headers: {
      Authorization: `Bearer ${token}`,
  ...
  if (!res.ok) throw new Error(text || `Agent error (${res.status})`);
